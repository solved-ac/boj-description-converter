...
function stringifyArray(
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (
      lp.isSpace(cur) ||
      lp.isSoftbreak(cur) ||
      lp.isCommandParameter(cur) ||
      lp.isSubscript(cur) ||
      lp.isSuperscript(cur)
    ) {
      continue;
    }
    ...
    if (
      i + 1 < len &&
      lp.isMathCharacter(arry[i + 1]) &&
      !lp.isMathCharacter(cur)
    ) {
      ret += " ";
      continue;
    }
    if (
      ...
      lp.isCommand(cur) &&
      cur.args.length === 0 &&
      lp.isCommandParameter(arry[i + 1])
    ) {
      ret += " ";
      continue;
    }
...
export function stringify(
  ...
  if (lp.isAmsMathTextCommand(node)) {
    return "\\text{" + stringifyArray(node.arg.content, options) + "}";
  }
  ...
  if (lp.isHrefCommand(node)) {
    ...
    if (node.arg) {
      return (
        "\\href" + stringify(node.arg, options) + `{${node.url}}{${content}}`
      );
    } else {
  ...
  if (
    lp.isEnvironment(node) ||
    lp.isMathEnv(node) ||
    lp.isMathEnvAligned(node)
  ) {
  ...
  if (lp.isMinted(node)) {
    ...
    return (
      "\\begin{minted}" + args + node.content + "\\end{minted}" + lineBreak
    );
  }
  ...
  if (lp.isDisplayMath(node)) {
    return (
      "\\[" +
      lineBreak +
      stringifyArray(node.content, options).trim() +
      lineBreak +
      "\\]" +
      lineBreak
    );
